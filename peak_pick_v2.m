function [Coef,simi]=peak_pick_v2(f_can,varargin)
...
if nargin>=5
    N_min=varargin{4};
    if length(N_min)~=length(f_can), error('N_max need to have the same length with f_can');end
end
if nargin>=6
    weight_Coef=varargin{5};
...
if nargin>=4, N_max=N_max(I); end
if nargin>=5, N_min=N_min(I); end
if nargin>=6, weight_Coef=weight_Coef(I); end
...
% if nargin>=2% when displayment is required
    ...
    [Coef,simi]=fre_combination_ILP(x,f_can_sorted,err,N_max,N_min,weight_Coef); 
    warning_str=sprintf("cannot find a solution at %0.2f",x);
    if sum(abs(Coef))==0, disp(warning_str);return; end
% else
